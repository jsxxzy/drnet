...
HTTP_API const char* ft_http_post_request(ft_http_client_t* http, const char* url, char* post_data, char* user_header)
{
	http->method = M_POST;
	http->download = 0;
	http->redirect = 0;
	...
	http->error_code = http_internal_sync_request(http, url, post_data, strlen(post_data), user_header, strlen(user_header));
